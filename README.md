...
- `gaming.html` – Gaming-Modul: Spielebibliothek mit Status (Wunschliste/Spiele ich/Pausiert/Durchgespielt/Abgebrochen), Plattform, Sterne-Bewertung, Filter nach Person und Status, sowie eine Statistik-Ansicht mit Spielzeit-Verlauf (Balkendiagramm der letzten 14 Tage), Bestenliste "Meistgespielt" und Sitzungs-Verlauf – Spielsitzungen werden über „▶ Spielsitzung erfassen" geloggt (Datum + Dauer)
- `handwerk.html` – Handwerk-Modul: Projekte mit Status (Idee/Geplant/In Arbeit/Fertig), Materialliste pro Projekt, Zuordnung "Wer arbeitet dran" (nutzt `personen.js`). Button „🛒 Material zum Einkauf" überträgt die Materialliste gesammelt in die aktive Einkaufsliste (inkl. Mengen-Zusammenführung, gleiches Prinzip wie bei Kochen)
