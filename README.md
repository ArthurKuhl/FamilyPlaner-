...
- `sync.js` enthält die komplette Sync-Logik, die Zugangsdaten des gemeinsamen Firebase-Projekts sowie ein globales Fehler-Banner: zeigt in jedem Modul eine kurze, sichtbare Meldung bei unerwarteten Fehlern, statt dass eine Aktion (z.B. Speichern) wortlos fehlschlägt
...
- `index.html` – Shell mit Navigation, Service-Worker-Registrierung, merkt sich den zuletzt geöffneten Bereich, leitet Kachel-Klicks von der Startseite an den passenden Bereich weiter, Gesamt-Backup (Export/Import aller Bereiche als Datei, Symbol 💾) mit automatischer Erinnerung (roter Punkt am Symbol, falls seit 30 Tagen kein Export gemacht wurde; mit Snooze-Funktion)
- `home.html` – Startseite/Dashboard: zeigt auf einen Blick die heutigen Termine, offene Einkaufslisten-Posten, die heutigen Sport-Einheiten, das heute im Wochenplan vorgesehene Rezept, aktuelle Garten-Wetterwarnungen (Frost/Hitze/Starkregen/Hagel, sofern kürzlich im Garten-Modul abgerufen) sowie eine Kurzfassung "X Pflanzen aktuell pflanzbar / Y erntbar" (berechnet direkt auf der Startseite, unabhängig vom Garten-Modul); liest direkt aus dem gemeinsamen Speicher, da alle Module denselben Ursprung teilen. Kacheln sind antippbar und wechseln direkt in den jeweiligen Bereich. Die Garten-Kachel verlinkt weiterhin ohne genaue Gießaufgaben-Zahl, da diese Berechnung eng mit den Live-Wetterdaten im Garten-Modul verzahnt ist
- `garten.html` – Gartenplaner-Modul (das ursprüngliche, vollständige Feature-Set), inkl. "🌾 Jetzt pflanzen & ernten"-Übersicht (unter 🧰 Werkzeuge): zeigt auf einen Blick, welche Pflanzen im aktuellen Monat direktgesät/ausgepflanzt bzw. geerntet werden können, basierend auf Aussaatkalender-Daten und einem neuen Erntezeiten-Datensatz
...
- `termine.html` – Termine-Modul: Kalender- und Agendaansicht, Wiederholungen (wöchentlich/monatlich/jährlich – z.B. für Geburtstage und Feiertage), Filter nach Person (Familie/Mama/Papa/Lev/Malia), Erinnerungen per Browser-Benachrichtigung (15 Min. bis 1 Tag vorher, gleiche Einschränkung wie bei der Wasser-Erinnerung: funktioniert solange die App im Hintergrund geöffnet ist), .ics-Export zum Importieren aller Termine in Google/Apple Kalender
- `einkauf.html` – Einkauf-Modul: mehrere Einkaufslisten, Kategorien-Gruppierung, A-Z/Z-A-Sortierung innerhalb jeder Kategorie, Mengen-Parsing bei der Schnelleingabe (z.B. „500g Mehl“), Favoriten-Vorschläge, automatisches Zusammenführen gleicher offener Posten (z.B. „300g Mehl“ + „200g Mehl“ → „500g Mehl“; nur bei gleicher Einheit), Kennzeichnung wer ein Produkt hinzugefügt hat (per „Ich bin“-Auswahl, nutzt die gemeinsame Personenliste aus `personen.js`), Vorlagen für wiederkehrende Einkäufe (📋-Symbol: aktuelle Liste als Vorlage speichern, später mit einem Klick wieder anwenden, inkl. Mengen-Zusammenführung) sowie eine automatische, geräte-lokale Erinnerung an einem einstellbaren Wochentag (Standard: Sonntag), falls Vorlagen vorhanden sind
- `kochen.html` – Kochen & Backen-Modul: Rezeptsammlung mit Portionsskalierung, Favoriten, Kategorie-Filter, A-Z/Z-A-Sortierung, frei vergebbare Tags (z.B. „italienisch“, „schnell“, „vegetarisch“) mit eigener Filterleiste, Wochenplan (Rezepte auf Wochentage legen, wochenweise navigierbar) mit gesammelter Zutaten-Übertragung in den Einkauf für alle geplanten Tage auf einmal (inkl. Mengen-Zusammenführung, z.B. wenn dasselbe Rezept mehrfach in der Woche geplant ist). Button „Zum Einkauf“ am einzelnen Rezept überträgt die (skalierten) Zutaten direkt in die aktive Einkaufsliste (funktioniert, weil beide Module denselben Browser-Speicher nutzen)
- `gesundheit.html` – Gesundheit & Sport-Modul: Tages-Tracking (Gewicht, Schlaf, Wasser, Stimmung) pro Person, Sport-Log mit Kategorien, Gewichtsverlauf als Mini-Chart, Wochenstatistik, Wasser-Tagesziel und Sport-Wochenziel pro Person mit Fortschrittsbalken (anpassbar über ✎), Nährstoffbedarf-Rechner (Grundumsatz/Gesamtumsatz nach Mifflin-St-Jeor, Makro- und grobe Mikronährstoff-Richtwerte), eine Krafttraining-Referenz mit Muskelgruppen und Übungsvorschlägen, eine Dehnübungs-Referenz gegen Verspannungen, Hohlkreuz und schlechte Haltung, eine tägliche Medikamenten-/Vitamin-Checkliste pro Person sowie eine Wasser-Erinnerung per Browser-Benachrichtigung (geräte-lokale Einstellung, wird bewusst nicht synchronisiert)
- `gaming.html` – Gaming-Modul: Spielebibliothek mit Status (Wunschliste/Spiele ich/Pausiert/Durchgespielt/Abgebrochen), Plattform, Sterne-Bewertung, Filter nach Person und Status, A-Z/Z-A-Sortierung, sowie eine Statistik-Ansicht mit Spielzeit-Verlauf (Balkendiagramm der letzten 14 Tage), Bestenliste "Meistgespielt" und Sitzungs-Verlauf – Spielsitzungen werden über „▶ Spielsitzung erfassen" geloggt (Datum + Dauer)
- `handwerk.html` – Handwerk-Modul: Projekte mit Status (Idee/Geplant/In Arbeit/Fertig), A-Z/Z-A-Sortierung, Materialliste pro Projekt, Zuordnung "Wer arbeitet dran" (nutzt `personen.js`). Button „🛒 Material zum Einkauf" überträgt die Materialliste gesammelt in die aktive Einkaufsliste (inkl. Mengen-Zusammenführung, gleiches Prinzip wie bei Kochen)
